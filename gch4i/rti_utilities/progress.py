...
prog_file = work_dir / f"gch4i_progress_{datetime.now().date().strftime('%Y%m%d')}.xlsx"
...
for g in groups:  # loop through gch4i_names to check for input, emi, and proxy files
    ...
    proxies_done = set(g_proxies) <= set(proxy_labels)  # all proxies present as files
    # create dictionary/row for gch4i_name and add to list of gch4i rows
    gch4i_row = {"gch4i_name": g,
    ...
    if not emis_done:  # create dict of missing emi files if any are missing
        ...
        missing_emis = [{"gch4i_name": g,
                         "emi_not_done": e}
                        for e in g_emis if e not in emi_labels]
        missing_emi_rows.extend(missing_emis)
    if not proxies_done:  # create dict of missing proxy files if any are missing
        missing_proxies = [{"gch4i_name": g,
                            "proxy_not_done": p}
                           for p in g_proxies if p not in proxy_labels]
...
with pd.ExcelWriter(prog_file, engine='xlsxwriter') as writer:
    ...
    pd.DataFrame.from_dict(gch4i_rows).to_excel(writer,
                                                index=False,
                                                sheet_name="progress")
    ...
    format2 = workbook.add_format({"bg_color": "#C6EFCE",
                                   "font_color": "#006100",
                                   "border": 1})
    prog_sheet.conditional_format(f"$A$2:$A${gch4i_len}",
                                  {"type": "no_blanks",
                                   "format": format0})
    prog_sheet.conditional_format(f"$B$2:$D${gch4i_len}",
                                  {"type": "cell",
                                   "criteria": '=',
                                   "value": False,
                                   "format": format1})
    ...
    pd.DataFrame.from_dict(missing_emi_rows).to_excel(writer,
                                                      index=False,
                                                      sheet_name="missing_emis")
    emi_sheet = writer.sheets['missing_emis']
    emi_sheet.conditional_format(f"$A$2:$B${len(missing_emi_rows) + 1}",
                                 {"type": "no_blanks",
                                  "format": format0})
    emi_sheet.autofit()
    ...
    proxy_sheet.conditional_format(f"$A$2:$B${len(missing_proxy_rows) + 1}",
                                   {"type": "no_blanks",
                                    "format": format0})
    proxy_sheet.autofit()
