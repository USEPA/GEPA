...
# resolution, output_path = _ID_TO_KWARGS["01"].values()
# %%
def task_create_area_raster(resolution: float, output_path: Annotated[Path, Product]):
    ...
    gepa_profile.profile.update(count=1)

    # get the number of cells
    ncells = np.multiply(*gepa_profile.arr_shape)
    # create an empty array of the right shape, assign each cell a unique value
    tmp_arr = np.arange(ncells, dtype=np.int32).reshape(gepa_profile.arr_shape)
    ...
    results = [
        ...
        for i, (s, v) in enumerate(
            shapes(tmp_arr[:, [0]], transform=gepa_profile.profile["transform"])
    ...
    area_gdf = (
        gpd.GeoDataFrame.from_features(results, crs=gepa_profile.profile["crs"])
        .to_crs("ESRI:102003")
        .assign(
    ...
    area_matrix = np.repeat(
        area_gdf.cell_area_sq_cm.values, repeats=gepa_profile.arr_shape[1]
    ).reshape(gepa_profile.arr_shape)

    # save the file for all other tasks to use
    with rasterio.open(output_path, "w", **gepa_profile.profile) as dst:
        dst.write(area_matrix, 1)


# %%
sesh = pytask.build(
    tasks=[task_create_area_raster(**kwargs) for _id, kwargs in _ID_TO_KWARGS.items()],
