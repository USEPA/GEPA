...
nlcd_grass_path = sector_data_dir_path / "nlcd/NLCD_2012_grass_binary_gepa.parquet"
...
def task_grassland_proxy(
    ...
    mtbs_burn_perimeters = gpd.read_file(mtbs_burn_perimeter_path).to_crs(4326)
            ...
            for state in unique_states:
                ...
                if len(year_state_proxy) == 0:
                    ...
                    final_proxy_df = pd.concat(
                        [final_proxy_df, dummy_row], ignore_index=True
                    )
                    continue
    ...
    grassland_proxy["fuelbed_aggregate"] = (
        grassland_proxy["fuelbed_aggregate"]
        .str.replace("evg", "")
        .astype(float)
        .astype(int)
    ...
    grassland_proxy_df = calculate_state_emissions(
        grassland_emi, grassland_proxy, years
    )
    ...
    grassland_proxy_df = grassland_proxy_df.merge(
        mtbs_burn_perimeters, on="eventID", how="left"
    )

    # only get the states we need for GHGI emissions
    grassland_proxy_df = grassland_proxy_df[
        grassland_proxy_df["state_code"].isin(state_gdf["state_code"])
    ]
    ...
    nlcd_gdf = (
        gpd.read_parquet(nlcd_grass_path)
    ...
    states_with_geometry = grassland_proxy_df[
        grassland_proxy_df["geometry"].isnull() == False
    ].copy()

    # States that are missing geometry data
    states_missing_geometry = (
        grassland_proxy_df[grassland_proxy_df["geometry"].isnull() == True]
        .copy()
        .loc[:, ["state_code", "year", "rel_emi"]]
        .drop_duplicates(subset=["state_code", "year"])
    )

    # Merge the state data with the NLCD geometry data
    states_missing_geometry = states_missing_geometry.merge(
        nlcd_gdf, on="state_code", how="left"
    )

    # Concatenate the two dataframes
    grassland_proxy_df = pd.concat(
        [states_with_geometry, states_missing_geometry], ignore_index=True
    )

    # %% Step 3 Create the final proxy dataframe

    # Check that the relative emissions sum to 1 for each state
    sums = grassland_proxy_df.groupby(["state_code", "year"])[
        "rel_emi"
    ].sum()  # get sums to check normalization
    assert np.isclose(
        sums, 1.0, atol=1e-8
    ).all(), f"Relative emissions do not sum to 1 for each year and state; {sums}"  # assert that the sums are close to 1

    final_grassland_proxy_df = grassland_proxy_df[
        ["state_code", "year", "rel_emi", "geometry"]
    ]

    final_grassland_proxy_df = gpd.GeoDataFrame(
        final_grassland_proxy_df, geometry="geometry"
    )

    # verify the geometries are valied
    # Fix invalid geometries
    final_grassland_proxy_df["geometry"] = final_grassland_proxy_df["geometry"].apply(
        lambda geom: make_valid(geom) if not geom.is_valid else geom
    )

    # Verify fix
    still_invalid = final_grassland_proxy_df[
        ~final_grassland_proxy_df.geometry.is_valid
    ]
    assert (
        len(still_invalid) == 0
    ), f"Invalid geometries still present in the proxy data: {still_invalid}"
