...
def task_get_ng_all_well_count_proxy_data(
    ...
    tank_vent_emi_path: Path = emi_data_dir_path / "tank_vent_emi.csv",
    ...
    *IL and IN do not report to Enverus, but do have oil and gas production. Production
    ...
    """
    Make Annual gridded arrays (maps) of well data (a well will be counted every
        month if there is any production that year)
    Includes NA Gas Wells and Production onshore in the CONUS region
    source emissions are related to the presence of a well and its production status
        (no emission if no production)
    ...
    Well Type (e.g., non-associated gas well) is determined based on annual production
    ...
    exp_well_emi_df = (pd.read_csv(non_assoc_exp_well_emi_path)
                       .query("state_code.isin(@state_gdf['state_code'])")
                       .query("ghgi_ch4_kt > 0.0")
                       )
    tank_vent_emi_df = (pd.read_csv(tank_vent_emi_path)
                        .query("state_code.isin(@state_gdf['state_code'])")
                        .query("ghgi_ch4_kt > 0.0")
                        )
    emi_df = pd.concat([exp_well_emi_df, tank_vent_emi_df]).reset_index(drop=True)
