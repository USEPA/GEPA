...
Input Files:            State Geo: global_data_dir_path / "tl_2020_us_state.zip"
                        ...
                        Emissions: emi_data_dir_path / "allwell_prod_emi.csv"
Output Files:           proxy_data_dir_path / "ng_all_well_prod_proxy.parquet"
"""
...
def task_get_ng_all_well_prod_proxy_data(
    ...
    allwell_prod_emi_path: Path = emi_data_dir_path / "allwell_prod_emi.csv",
    ...
    *IL and IN do not report to Enverus, but do have oil and gas production. Production
    ...
    # Active gas well (conventional + HF) gas production in a given month
    all_well_prod_df = pd.DataFrame()
    ...
    for iyear in years:
        ...
        ng_data_temp = (
            pd.read_csv(enverus_file_path_iyear,
                        dtype={3: 'str', 'spud_year': str, 'first_prod_year': str})
        ...
        for imonth in range(1, 13):
            ...
            all_well_prod_imonth = (
                ng_data_imonth_temp[['year', 'month', 'year_month', 'STATE_CODE', 'LATITUDE', 'LONGITUDE', gas_prod_str]]
    ...
    all_well_prod_df['empty_geometry'] = all_well_prod_df.is_empty
    print("Number of total data entries: ", len(all_well_prod_df))
    print("Number of data entries with missing geometry: ", len(all_well_prod_df.query("empty_geometry == True")))
    all_well_prod_df = all_well_prod_df.query("empty_geometry == False").drop(columns="empty_geometry").reset_index(drop=True)
    ...
    all_well_prod_df = pd.concat([all_well_prod_df, nei_df]).astype({'year': int}).reset_index(drop=True)
    ...
    emi_df = (pd.read_csv(allwell_prod_emi_path)
                          .query("state_code.isin(@state_gdf['state_code'])")
    ...
    proxy_states = set(all_well_prod_df[['state_code', 'year']].itertuples(index=False, name=None))

    # Find missing states
    missing_states = emi_states.difference(proxy_states)

    # Add missing states alternative data to grouped_proxy
    proxy_gdf_final = create_alt_proxy(missing_states, all_well_prod_df)
