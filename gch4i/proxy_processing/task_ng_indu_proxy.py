...
# file
state_path_unzipped: Path = global_data_dir_path / "tl_2020_us_state" / 'tl_2020_us_state.shp'

@mark.persist
@task(id='ng_indu_proxy')
def task_get_reporting_ng_indu_proxy_data(subpart_C=GHGRP_subC_inputfile,
    subpart_D=GHGRP_subD_inputfile,
    facility_path=GHGRP_subDfacility_loc_inputfile,
    state_path=state_path_unzipped,
    reporting_ng_indu_proxy_output_path: Annotated[Path, Product] = proxy_data_dir_path / 'ng_indu_proxy.parquet'
):
    
    proxy_gdf = create_raw_indu_proxy(subpart_C, subpart_D, facility_path, reporting_ng_indu_proxy_output_path)
    
    #filter proxy_gdf so it only contains points which fall within the continental US geometry
    continental_us_gdf = ( # get continential US geometries from state geometries
        ...
        )[['geometry']] #select only the geometry column for spatial filtering
    )
    proxy_gdf = gpd.sjoin(proxy_gdf, continental_us_gdf, how="inner", predicate="within").drop(columns=['index_right'])
