"""
Name:                   task_oil_basin_430_prod_proxy.py
Date Last Modified:     2025-03-21
Authors Name:           Hannah Lohman (RTI International)
Purpose:                Mapping of oil well production emissions in Basin 430.
Input Files:            State Geo: global_data_dir_path / "tl_2020_us_state.zip"
                        ...
                        Emissions: emi_data_dir_path / "petr_basin_430_emi.csv"
Output Files:           proxy_data_dir_path / "oil_basin_430_prod_proxy.parquet"
"""
    ...
    *IL and IN do not report to Enverus, but do have oil and gas production. Production
    ...
    for iyear in years:
        ...
        for imonth in range(1,13):
            ...
            basin_430_prod_imonth = (oil_data_imonth_temp[['year', 'month','year_month','STATE_CODE','LATITUDE','LONGITUDE','AAPG_CODE_ERG',oil_prod_str]]
    ...
    basin_430_prod_df['empty_geometry'] = basin_430_prod_df.is_empty
    print("Number of total data entries: ", len(basin_430_prod_df))
    print("Number of data entries with missing geometry: ", len(basin_430_prod_df.query("empty_geometry == True")))
    basin_430_prod_df = basin_430_prod_df.query("empty_geometry == False").drop(columns="empty_geometry").reset_index(drop=True)
