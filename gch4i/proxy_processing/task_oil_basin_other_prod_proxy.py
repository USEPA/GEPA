"""
Name:                   task_oil_basin_other_prod_proxy.py
Date Last Modified:     2025-03-21
Authors Name:           Hannah Lohman (RTI International)
Purpose:                Mapping of oil well production emissions in Other Basins.
Input Files:            State Geo: global_data_dir_path / "tl_2020_us_state.zip"
                        ...
                        Emissions: emi_data_dir_path / "petr_basin_other_emi.csv"
Output Files:           proxy_data_dir_path / "oil_basin_other_prod_proxy.parquet"
"""
...
def task_get_oil_basin_other_prod_proxy_data(
    ...
    basin_other_emi_path: Path = emi_data_dir_path / "petr_basin_other_emi.csv",
    ...
    *IL and IN do not report to Enverus, but do have oil and gas production. Production
    ...
    for iyear in years:
        ...
        for imonth in range(1,13):
            ...
            basin_other_prod_imonth = (oil_data_imonth_temp[['year', 'month','year_month','STATE_CODE','LATITUDE','LONGITUDE','AAPG_CODE_ERG',oil_prod_str]]
    ...
    basin_other_prod_df = basin_other_prod_df.astype({'year': int})
    ...
    basin_other_prod_df = pd.concat([basin_other_prod_df, nei_df]).astype({'year': int}).reset_index(drop=True)
