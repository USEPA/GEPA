...
Input Files:            State Geo: global_data_dir_path / "tl_2020_us_state.zip"
                        ...
                        Emissions: emi_data_dir_path / "oil_prod_emi.csv"
                        Emissions: emi_data_dir_path / "trans_onshore_emi.csv"
Output Files:           proxy_data_dir_path / "oil_all_well_prod_proxy.parquet"
"""
...
def task_get_oil_all_well_prod_proxy_data(
    ...
    *IL and IN do not report to Enverus, but do have oil and gas production. Production
    ...
    trans_onshore_emi_df = (pd.read_csv(trans_onshore_emi)
                            .query("state_code.isin(@state_gdf['state_code'])")
                            .query("ghgi_ch4_kt > 0.0")
                            )
    emi_df = pd.concat([oil_prod_emi_df, trans_onshore_emi_df]).reset_index(drop=True)
