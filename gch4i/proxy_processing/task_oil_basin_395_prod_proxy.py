"""
Name:                   task_oil_basin_395_prod_proxy.py
...
Input Files:            State Geo: global_data_dir_path / "tl_2020_us_state.zip"
                        ...
                        Emissions: emi_data_dir_path / "petr_basin_395_emi.csv"
Output Files:           proxy_data_dir_path / "oil_basin_395_prod_proxy.parquet"
"""
...
def task_get_oil_basin_395_prod_proxy_data(
    ...
    basin_395_emi_path: Path = emi_data_dir_path / "petr_basin_395_emi.csv",
    ...
    *IL and IN do not report to Enverus, but do have oil and gas production. Production
    ...
    for iyear in years:
        ...
        for imonth in range(1,13):
            ...
            basin_395_prod_imonth = (oil_data_imonth_temp[['year', 'month','year_month','STATE_CODE','LATITUDE','LONGITUDE','AAPG_CODE_ERG',oil_prod_str]]
    ...
    basin_395_prod_df['empty_geometry'] = basin_395_prod_df.is_empty
    print("Number of total data entries: ", len(basin_395_prod_df))
    print("Number of data entries with missing geometry: ", len(basin_395_prod_df.query("empty_geometry == True")))
    basin_395_prod_df = basin_395_prod_df.query("empty_geometry == False").drop(columns="empty_geometry").reset_index(drop=True)

    # Calculate relative emissions and convert to a geodataframe
    basin_395_prod_df = calc_enverus_rel_emi(basin_395_prod_df)
    basin_395_prod_df = basin_395_prod_df.astype({'year': int})
    ...
    emi_df = (pd.read_csv(basin_395_emi_path)
                          .query("state_code.isin(@state_gdf['state_code'])")
    ...
    proxy_states = set(basin_395_prod_df[['state_code', 'year']].itertuples(index=False, name=None))

    # Find missing states
    missing_states = emi_states.difference(proxy_states)

    # Add missing states alternative data to grouped_proxy
    proxy_gdf_final = create_alt_proxy(missing_states, basin_395_prod_df)

    # Output Proxy Parquet Files
    proxy_gdf_final.to_parquet(basin_395_prod_output_path)
