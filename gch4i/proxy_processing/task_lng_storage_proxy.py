"""
Name:                  task_lng_storage_proxy.py
Date Last Modified:    2025-01-23
Authors Name:          John Bollenbacher (RTI International)
Purpose:               Creates proxy data for LNG storage facilities using PHMSA,
                        Enverus, and FracTracker data.
Input Files:           - LNG Storage Input Folder: {sector_data_dir_path}/lng/annual-
                        liquefied-natural-gas-2010-present/
                       - LNG Storage Enverus Input File: {sector_data_dir_path}/lng/
                        LNG_Terminals_AllUS_WGS84.xls
                       - FracTracker Input File: {sector_data_dir_path}/lng/
                        FracTracker_PeakShaving_WGS84.xls
Output Files:          - {proxy_data_dir_path}/lng_storage_proxy.parquet
"""
...
def get_lng_storage_proxy_data(
    ...
    for file_path in phmsa_files:
        ...
        df = df[['REPORT_YEAR', 'FACILITY_NAME', 'PARTA2NAMEOFCOMP',
                 'TOTAL_CAPACITY_BBLS', 'TOTAL_CAPACITY_MMCF', 'FACILITY_STATE',
                 'FACILITY_ZIP_CODE', 'FACILITY_STATUS', 'TYPE_OF_FACILITY',
                 'FUNCTION_OF_FACILITY']]
        # filter for storage facilities that are in service
        df = df[
            df['FUNCTION_OF_FACILITY'].isin(['Storage w/ Liquefaction',
                                             'Storage w/o Liquefaction',
                                             'Storage w/ Both'])
            &
    ...
    # initialize Lat and Lon columns
    phmsa_df['Lat'] = np.nan
    phmsa_df['Lon'] = np.nan
    # filter by report year, and check that all years are present
    phmsa_df = phmsa_df[phmsa_df['REPORT_YEAR'].isin(range(min_year, max_year + 1))]
    ...
    enverus_df = pd.read_excel(
        ...
        usecols=['NAME', 'OPERATOR', 'TYPE', 'CAP_STO', 'STATE_NAME', 'CNTY_NAME',
                 'Longitude', 'Latitude']
    )
    # Remove Alaska and Hawaii
    enverus_df = (enverus_df[~enverus_df['STATE_NAME'].isin(['Alaska', 'Hawaii'])]
    ...
    # Remove Alaska and Hawaii
    fractracker_df = (fractracker_df[~fractracker_df['State']
                      .isin(['Alaska', 'Hawaii'])]
                      .reset_index(drop=True))
    # clean up zip codes
    fractracker_df['Zip'] = (fractracker_df['Zip']
                             .astype(str)
                             .apply(lambda x: x.replace('.', '0').zfill(5)))
    ...
    def clean_facility_name(name):
        """
        Clean up facility names for matching.
        """
        return str(name).lower().replace("lng", "").strip()

    # Get unique facilities
    unique_phmsa_facilities = (phmsa_df[['FACILITY_NAME', 'PARTA2NAMEOFCOMP',
                                         'FACILITY_STATE', 'FACILITY_ZIP_CODE']]
                               .drop_duplicates().reset_index(drop=True))

    # Match facilities with Enverus data
    ...
    # apply manual matches to dataframe
    for idx in unique_phmsa_facilities.index:
        facility = unique_phmsa_facilities.loc[idx]
        state = facility['FACILITY_STATE']
    ...
    proxy_gdf = gpd.GeoDataFrame(
        ...
        .groupby(['facility_name', 'company_name', 'year',
                  'state_code', 'geometry'])['rel_emi'].sum().reset_index())
