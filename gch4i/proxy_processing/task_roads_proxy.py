...
def task_roads_proxy() -> None:
    ...
    for year in range(min_year, max_year + 1):
        ...
        interm_out_path = (
            task_outputs_path / f"gridwise_mapping_intermediate_{year}.csv"
        )
        ...
        if not interm_out_path.exists():
            ...
            ).to_csv(interm_out_path, index=False)
            ...
            cells = cells.loc[
                ~cells.set_index(["cell_id", "state", "region"]).index.isin(
                    processed_cells.set_index(["cell_id", "state", "region"]).index
                )
            ]
        ...
        for idx, cell in cells.reset_index().iterrows():
            ...
            geom = cell[
                "geometry"
            ]  # get box around cell for more efficient spatial query

            # get road value from proxy data
            road_value = road_proxy_df.loc[
                (road_proxy_df["state"] == state)
                & (road_proxy_df["region"] == region)
                & (road_proxy_df["year"] == year)
            ]
            ...
            try:
                ...
                try:
                    ...
                    print(
                        f"ERROR processing cell {cell_id}, region: {region}, state: "
                        f"{state}, for year {year}: {e}"
                    )
                    continue
            ...
            if not cell_roads.empty:
                road_type_df = (
                    cell_roads.groupby(road_type_col, observed=True)["geometry"]
                ...
                road_type_df = (
                    pd.DataFrame(
                        {road_type_col: roads[road_type_col].unique(), "rd_length": 0}
                    )
                    .assign(
                        state=state,
                        region=region,
            ...
            road_value.set_index(["state", "region", "year", "road_type"]).join(
                road_type_df, how="left"
            ).reset_index().to_csv(interm_out_path, mode="a", header=False, index=False)
        ...
        state_road_length = (
            long_table.groupby(["state", "year", "road_type", "region"])["rd_length"]
            .sum()
            .rename("m_road_in_state")
            .reset_index()
        )

        # join with long table:
        long_table = long_table.join(
            state_road_length.set_index(["state", "year", "road_type", "region"]),
            on=["state", "year", "road_type", "region"],
        )

        # calculate methane emission allocation at the cell/region level
        long_table["methane_emission_allocation"] = (
            long_table["rd_length"] / long_table["m_road_in_state"]
        ) * long_table["proxy"]

        print(f"Saving to CSV... {datetime.now()}")
        long_table.to_csv(
            task_outputs_path / f"gridwise_mapping_{year}.csv", index=False
        )
        ...
        print(
            "The long table should not sum to 1 bc the proxy value has been allocated "
            "to each urban/rural and road type in each cell"
        )
        print(long_table.groupby(["vehicle", "state", "year"]).proxy.sum())

        print(
            "For example, here is one cell's allocation of proxies, which should sum "
            "to 1 if all 3 road_types are present in both urban/rural region"
        )
        example = long_table.query(
            "state=='WA' & year==2017 & vehicle=='Passenger' & cell_id==42072.0"
        )
        print(example)
        ...
        print(
            long_table.groupby(by="state")["methane_emission_allocation"]
            .sum()
            .sort_values()
        )
        ...
        # state-wide methane emissions from that year that are attirbutable to that
        # vehicle type/road/urbanicity(region) type in a given cell
        cell_csv_path = task_outputs_path / f"gridwise_mapping_{year}_cell.csv"
        long_table.groupby(["cell_id"])[
            "methane_emission_allocation"
        ].sum().reset_index().to_csv(cell_csv_path, index=False)
    ...
    long_data = pd.concat(
        [
            pd.read_csv(task_outputs_path / f"gridwise_mapping_{year}_cell.csv")
            .groupby(["cell_id"])["methane_emission_allocation"]
            .sum()
            .rename(year)
            for year in years
        ],
    ...
    cells_proxy["y"] = (
        cells_proxy["geometry"].apply(lambda geom: geom.exterior.coords.xy[1][0]) - 0.1
    )
    cells_proxy["x"] = (
        cells_proxy["geometry"].apply(lambda geom: geom.exterior.coords.xy[0][0]) + 0.1
    )

    # sort by latitude first, then longitude
    # this will create a grid that is ordered by longitude, with the first row being
    # the lowest latitude
    cells_proxy = cells_proxy.sort_values(["y", "x"], ascending=[False, True])

    # extract the array for that year and reshape using the original profile's height
    # and width this will create an array with the same dimensions as the original grid
    proxy_array = (
        cells_proxy[years]
        .to_numpy()
        .reshape(profile.profile["height"], profile.profile["width"], len(years))
    )  # ], order='F')
    lat_array = (
        cells_proxy["y"]
        .to_numpy()
        .reshape(profile.profile["height"], profile.profile["width"])
    )  # ], order='F')
    lon_array = (
        cells_proxy["x"]
        .to_numpy()
    ...
    ds = xr.Dataset(
        ...
        coords={
            ...
            "year": (
                ["year"],
                years,
            ),  # Assuming year_array has a third dimension for years
        },
    )

    ds["rel_emi"].sel(year=2018).plot()
    ...
    state_grid = state_grid.assign_coords({"x": ds.coords["x"], "y": ds.coords["y"]})
