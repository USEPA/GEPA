"""
Name:                   task_oil_hf_well_count_proxy.py
Date Last Modified:     2025-07-09
Authors Name:           Hannah Lohman (RTI International)
Purpose:                Mapping of natural oil HF well count proxy emissions.
Input Files:            State Geo: global_data_dir_path / "tl_2020_us_state.zip"
                        ...
                        Emissions: emi_data_dir_path / "pet_hf_oilwell_emi.csv"
Output Files:           proxy_data_dir_path / "oil_hf_well_count_proxy.parquet"
"""
...
def task_get_oil_hf_well_count_proxy_data(
    ...
    emi_path: Path = emi_data_dir_path / "pet_hf_oilwell_emi.csv",
    ...
    *IL and IN do not report to Enverus, but do have oil and gas production. Production
    ...
    for iyear in years:
        ...
        for imonth in range(1,13):
            ...
            # HF Well Count
            hf_well_count_imonth = (oil_data_imonth_temp[['year', 'month','year_month','STATE_CODE','LATITUDE','LONGITUDE','WELL_COUNT','HF']]
                                   .query("HF == 'Y'")
                                   .drop(columns=["HF"])
    ...
    hf_well_count_df['empty_geometry'] = hf_well_count_df.is_empty
    print("Number of total data entries: ", len(hf_well_count_df))
    print("Number of data entries with missing geometry: ", len(hf_well_count_df.query("empty_geometry == True")))
    hf_well_count_df = hf_well_count_df.query("empty_geometry == False").drop(columns="empty_geometry").reset_index(drop=True)

    # Calculate relative emissions and convert to a geodataframe
    hf_well_count_df = calc_enverus_rel_emi(hf_well_count_df)
    hf_well_count_df = hf_well_count_df.astype({'year': int})
    ...
    hf_well_count_df = pd.concat([hf_well_count_df, nei_df]).astype({'year': int}).reset_index(drop=True)
    ...
    proxy_states = set(hf_well_count_df[['state_code', 'year']].itertuples(index=False, name=None))

    # Find missing states
    missing_states = emi_states.difference(proxy_states)

    # Add missing states alternative data to grouped_proxy
    proxy_gdf_final = create_alt_proxy(missing_states, hf_well_count_df)
    ...
    oil_conv_well_count_proxy = gpd.read_parquet(oil_conv_well_count_proxy_path)
    ...
    proxy_gdf_final.to_parquet(hf_well_count_output_path)
