"""
Name:                   task_oil_conv_well_count_proxy.py
Date Last Modified:     2025-03-21
Authors Name:           Hannah Lohman (RTI International)
Purpose:                Mapping of oil conventional well count proxy emissions.
Input Files:            State Geo: global_data_dir_path / "tl_2020_us_state.zip"
                        ...
                        Emissions: emi_data_dir_path / "conv_oil_well_emi.csv"
Output Files:           proxy_data_dir_path / "oil_conv_well_count_proxy.parquet"
"""
...
def task_get_oil_conv_well_count_proxy_data(
    ...
    conv_oil_well_emi_path: Path = emi_data_dir_path / "conv_oil_well_emi.csv",
    ...
    *IL and IN do not report to Enverus, but do have oil and gas production. Production
    ...
    for iyear in years:
        ...
        for imonth in range(1,13):
            ...
            conv_well_count_imonth = (oil_data_imonth_temp[['year', 'month','year_month','STATE_CODE','LATITUDE','LONGITUDE','WELL_COUNT','HF']]
                                     .query("HF != 'Y'")
                                     .drop(columns=["HF"])
    ...
    conv_well_count_df['empty_geometry'] = conv_well_count_df.is_empty
    print("Number of total data entries: ", len(conv_well_count_df))
    print("Number of data entries with missing geometry: ", len(conv_well_count_df.query("empty_geometry == True")))
    conv_well_count_df = conv_well_count_df.query("empty_geometry == False").drop(columns="empty_geometry").reset_index(drop=True)

    # Calculate relative emissions and convert to a geodataframe
    conv_well_count_df = calc_enverus_rel_emi(conv_well_count_df)
    conv_well_count_df = conv_well_count_df.astype({'year': int})
    # NEI Data:
    nei_df = pd.DataFrame()
    ...
    conv_well_count_df = pd.concat([conv_well_count_df, nei_df]).astype({'year': int}).reset_index(drop=True)
    ...
    emi_df = (pd.read_csv(conv_oil_well_emi_path)
                          .query("state_code.isin(@state_gdf['state_code'])")
    ...
    proxy_states = set(conv_well_count_df[['state_code', 'year']].itertuples(index=False, name=None))

    # Find missing states
    missing_states = emi_states.difference(proxy_states)

    # Add missing states alternative data to grouped_proxy
    proxy_gdf_final = create_alt_proxy(missing_states, conv_well_count_df)

    # Output Proxy Parquet Files
    proxy_gdf_final.to_parquet(conv_well_count_output_path)
