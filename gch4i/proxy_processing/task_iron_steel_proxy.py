...
def task_get_iron_steel_proxy_data(
    ...
    ghgi_facilities_df = (
        ghgi_facilities_df.rename(
            columns={"state": "state_code", "years operated": "years_operated"}
        )
        .drop(
    ...
    ghgi_facilities_df = pd.merge(
        ghgi_facilities_df[
            ["facility_name", "address1", "state_code", "years_operated"]
        ],
    ...
    fill_missing = unmatched_df[["facility_name", "address1"]].merge(
    ...
    ghgi_facilities_df = ghgi_facilities_df.merge(
        fill_missing, on="address1", how="left", suffixes=("_x", "_y")
    )
    ghgi_facilities_df = (
        ghgi_facilities_df.assign(
            facility_id=(
                ghgi_facilities_df["facility_id_y"].fillna(
                    ghgi_facilities_df["facility_id_x"]
                )
            )
        )
    ...
    # Insert ID:: 1014686
    ghgi_facilities_df.loc[
        ghgi_facilities_df["facility_name"] == "Steel Dynamics Southwest, LLC",
        "facility_id",
    ] = 1014686
    ...
    # Insert ID:: 1008263
    ghgi_facilities_df.loc[
        ghgi_facilities_df["facility_name"] == "Kentucky Electric Steel Company",
        "facility_id",
    ] = 1008263
    ...
    ghgi_facilities_df["years_operated"] = ghgi_facilities_df["years_operated"].apply(
        extract_years
    )
    ghgi_facilities_df["year"] = ghgi_facilities_df["years_operated"].apply(
        expand_years
    )
    ghgi_facilities_df = (
        ghgi_facilities_df.explode("year")
        .drop(columns=["years_operated"])
        .reset_index(drop=True)
    )

    # Subset subpart Q data to unique facility_id and year
    subpart_Q_red = subpart_Q.drop_duplicates(
        subset=["facility_id", "year"], keep="first"
    )
    ...
    full_proxy_df = proxy_df[proxy_df["ghg_quantity"] > 0].copy()

    # Step 1: Calculate the sum of emissions for each state-year group
    full_proxy_df["total_emissions"] = full_proxy_df.groupby(["state_code", "year"])[
        "ghg_quantity"
    ].transform("sum")

    # Step 2: Normalize the emissions for each facility
    full_proxy_df["rel_emi"] = (
        full_proxy_df["ghg_quantity"] / full_proxy_df["total_emissions"]
    )
    ...
    proxy_gdf = (
        gpd.GeoDataFrame(
            full_proxy_df.drop(columns=["latitude", "longitude"]),
    ...
    emi_df = pd.read_csv(emi_iron_steel_path).query("ghgi_ch4_kt != 0")
    ...
    proxy_states = set(
        proxy_gdf[["state_code", "year"]].itertuples(index=False, name=None)
    )
    ...
    if missing_states:
        ...
        missing_facilities = proxy_df.query(
            "state_code in @unique_states"
        ...
        alt_proxy = gpd.GeoDataFrame(
            missing_rows_df.drop(columns=["latitude", "longitude"]),
            geometry=gpd.points_from_xy(
        ...
        proxy_gdf = pd.concat([proxy_gdf, alt_proxy], ignore_index=True)  # .sjoin(
        # state_gdf[["geometry", "state_code"]], how="left"
        # )
