...
# NEI Lat/Lon Path
#loc_path = '/Users/aburnette/Library/CloudStorage/OneDrive-SharedLibraries-EnvironmentalProtectionAgency(EPA)/Gridded CH4 Inventory - RTI 2024 Task Order/Task 2/ghgi_v3_working/GEPA_Source_Code/Global_InputData/Gridded/NEI_Reference_Grid_LCC_to_WGS84_latlon.shp'
loc_path = V3_DATA_PATH / "sector" / "nei_og" / "NEI_Reference_Grid_LCC_to_WGS84_latlon.shp"
# Read in shape file for Lat/Lon
shape = gpd.read_file(loc_path)

# Enverus Path: Base Proxy Data
Enverus_path = V3_DATA_PATH.parent / "GEPA_Source_Code" / "Global_InputData" / "Enverus" / "AOG" / "DIDSK_HEADERS_API10_2019_abandoned_wells.csv"
...
def get_aog_wells_proxy_data(
        filter_condition,
        state_path: Path = global_data_dir_path / "tl_2020_us_state.zip",
        loc_path: Path = V3_DATA_PATH / "sector" / "nei_og" / "NEI_Reference_Grid_LCC_to_WGS84_latlon.shp",
        Enverus_path: Path = V3_DATA_PATH.parent / "GEPA_Source_Code" / "Global_InputData" / "Enverus" / "AOG" / "DIDSK_HEADERS_API10_2019_abandoned_wells.csv",
    ...
    location_data = (
        ...
        # Drop columns
        .drop(columns=['API10', 'COUNTYPARISH', 'CUM_GAS', 'CUM_OIL', 'WELL_STATUS',
                       'PRODUCTION_TYPE', 'ERG_WELL_TYPE', 'GOR'])
        # Rename columns
        .rename({'SURFACE_HOLE_LATITUDE_WGS84': 'LAT',
                'SURFACE_HOLE_LONGITUDE_WGS84': 'LON'}, axis=1)
        # Filter data
        .query('OFFSHORE == "N"')
        .query('STATE in @state_list.state_code')
    ...
    for year in year_range:
        ...
        abandoned_wells = location_data[
            ...
            ((location_data['prod_year'].isna()) & (location_data['comp_year'] < year)) |
        ...
        well_counts = (
            abandoned_wells.groupby(['state', 'abandoned_well_type'])['producing_entity_count']
            .sum()
        ...
        well_counts['gas_to_oil_ratio'] = well_counts['GAS'] / (well_counts['GAS'] + well_counts['OIL']).replace(0, np.nan)
        ...
        year_results = year_results[year_results['state'].isin(state_list['state_code'])]
    ...
    for year in year_range:
        ...
        temp = location_data_corr[
            ...
            ((location_data_corr['prod_year'].isna()) & (location_data_corr['comp_year'] < year)) |
        ...
        temp_gas = (
            temp.query('abandoned_well_type == "DRY"')
            .assign(
                abandoned_well_type='GAS',
                producing_entity_count=lambda x: x['producing_entity_count'] * x['gas_to_oil_ratio']
            )
        )
        ...
        temp_oil = (
            ...
            .assign(
                ...
                producing_entity_count=lambda x: x['producing_entity_count'] * (1 - x['gas_to_oil_ratio'])
            )
        )
    ...
    for iyear in np.arange(0, num_years):
        ...
        if year >= 2011 and year <= 2017:
            ...
            temp_result = (
                gpd.GeoDataFrame(
                    ...
                    geometry=gpd.points_from_xy(
                        temp_result['Longitude'],
                        temp_result['Latitude']
                        ),
                    crs=4326
                )
            ...
            temp_result = (
                ...
                .assign(
                    ...
                    abandoned_well_type=filter_condition
                    )
                )
            # Append to IL_IN_adj
        ...
        # If year is 2018-2022, data is in shapefile format
            ...
            if year <= 2020:
                temp_result = temp_result.filter(['ACTIVITY', 'state_code', 'geometry', 'year', 'abandoned_well_type'])
                temp_result = temp_result.rename(columns={'ACTIVITY': 'producing_entity_count'})
            elif year == 2021:
                temp_result = temp_result.filter(['NUMBER_', 'state_code', 'geometry', 'year', 'abandoned_well_type'])
                temp_result = temp_result.rename(columns={'NUMBER_': 'producing_entity_count'})
            elif year == 2022:
                temp_result = temp_result.filter(['NUMBER_WEL', 'state_code', 'geometry', 'year', 'abandoned_well_type'])
                temp_result = temp_result.rename(columns={'NUMBER_WEL': 'producing_entity_count'})
            # If year is not in range, print error
            else:
    ...
    proxy_gdf = pd.concat([base_results,
                           IL_IN_adj], ignore_index=True)
    ...
    proxy_gdf = (
        ...
        .assign(
            group_sum=lambda x: x.groupby(['state_code', 'year', 'abandoned_well_type'])['producing_entity_count'].transform('sum')
        )
    ...
    grouped_gdf = (
        proxy_gdf
        # Sum emissions
        .groupby(["state_code", "year", "abandoned_well_type", "geometry"], as_index=False)
        .agg({"producing_entity_count": "sum"})
        # Calculate state relative emissions (emissions / state_year emissions)
        .assign(
            state_year_sum=lambda x: x.groupby(['state_code', 'year', 'abandoned_well_type'])['producing_entity_count'].transform('sum'),
    ...
    proxy_dict = {
        'GAS': 'aog_gas_wells_emi',
        'OIL': 'aog_oil_wells_emi'
    }
    ...
    proxy_states = set(filtered_proxy[['state_code', 'year']].itertuples(index=False, name=None))
