...
def task_forest_land_proxy_data(
    ...
    forestland_proxy_df = forestland_proxy_df.merge(
    ...
    states_with_geometry = forestland_proxy_df[
        forestland_proxy_df["geometry"].isnull() == False
    ].copy()

    # States that are missing geometry data
    states_missing_geometry = (
        forestland_proxy_df[forestland_proxy_df["geometry"].isnull() == True]
        .copy()
    ...
    sums = forestland_proxy_df.groupby(["state_code", "year"])[
        "rel_emi"
    ...
    final_forestland_proxy_df = gpd.GeoDataFrame(
        final_forestland_proxy_df, geometry="geometry"
    )

    # verify the geometries are valied
    # Fix invalid geometries
    final_forestland_proxy_df["geometry"] = final_forestland_proxy_df["geometry"].apply(
        lambda geom: make_valid(geom) if not geom.is_valid else geom
    )

    # Verify fix
    still_invalid = final_forestland_proxy_df[
        ~final_forestland_proxy_df.geometry.is_valid
    ]
    assert (
        len(still_invalid) == 0
