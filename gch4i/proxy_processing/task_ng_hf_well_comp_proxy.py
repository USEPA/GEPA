...
Purpose:                Mapping of natural gas HF well completions proxy emissions.
Input Files:            State Geo: global_data_dir_path / "tl_2020_us_state.zip"
                        ...
                        Emissions: emi_data_dir_path / "non_assoc_exp_hf_comp_emi.csv"
Output Files:           proxy_data_dir_path / "ng_hf_well_comp_proxy.parquet"
"""
...
def task_get_ng_hf_well_comp_proxy_data(
    ...
    ng_hf_well_count_proxy_path: Path = proxy_data_dir_path / "ng_hf_well_count_proxy.parquet",
    ...
    *IL and IN do not report to Enverus, but do have oil and gas production. Production
    ...
    for iyear in years:
        ...
        for imonth in range(1,13):
            ...
            # HF Well Completions
            hf_well_comp_imonth = (ng_data_imonth_temp[['year', 'month','year_month','STATE_CODE','LATITUDE','LONGITUDE','WELL_COUNT','HF','comp_year_month']]
                                    .query("HF == 'Y'")
                                    .drop(columns=["HF"])
    ...
    # 3. Assign proxy data from the ng_hf_well_count_proxy to the remaining state-year
    ...
    # Add in ng_hf_well_count_proxy data to cover the remaining missing state-year combinations
    ng_hf_well_count_proxy = gpd.read_parquet(ng_hf_well_count_proxy_path)
    for istate_year in range(0, len(missing_states)):
        ...
        iproxy_data = ng_hf_well_count_proxy.query("state_code == @istate").query("year == @iyear")
