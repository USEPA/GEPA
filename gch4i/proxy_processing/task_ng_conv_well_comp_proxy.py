...
Purpose:                Mapping of natural gas conventional well completions proxy emissions.
Input Files:            State Geo: global_data_dir_path / "tl_2020_us_state.zip"
                        ...
                        Emissions: emi_data_dir_path / "non_assoc_exp_conv_comp_emi.csv"
...
def task_get_ng_conv_well_comp_proxy_data(
    ...
    *IL and IN do not report to Enverus, but do have oil and gas production. Production
    ...
    conv_well_comp_df['empty_geometry'] = conv_well_comp_df.is_empty
    print("Number of total data entries: ", len(conv_well_comp_df))
    print("Number of data entries with missing geometry: ", len(conv_well_comp_df.query("empty_geometry == True")))
    conv_well_comp_df = conv_well_comp_df.query("empty_geometry == False").drop(columns="empty_geometry").reset_index(drop=True)
    ...
    conv_well_comp_df = pd.concat([conv_well_comp_df, nei_df]).astype({'year': int}).reset_index(drop=True)
    ...
    emi_df = (pd.read_csv(non_assoc_exp_conv_comp_emi_path)
                          .query("state_code.isin(@state_gdf['state_code'])")
