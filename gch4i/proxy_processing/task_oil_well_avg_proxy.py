...
def task_get_oil_well_avg_proxy_data(
    ...
    well_avg_gdf = (gpd.read_parquet(oil_all_well_prod_proxy_path)
                    .rename(columns={'annual_rel_emi': 'prod_annual_rel_emi'})
    ...
    well_avg_gdf['count_annual_rel_emi'] = well_avg_gdf.groupby(['state_code', 'year'])['well_count'].transform(lambda x: x / x.sum() if x.sum() > 0 else 0)
    well_avg_gdf['count_rel_emi'] = well_avg_gdf.groupby(['state_code', 'year_month'])['well_count'].transform(lambda x: x / x.sum() if x.sum() > 0 else 0)
    well_avg_gdf = well_avg_gdf.drop(columns='well_count')

    # Check that relative emissions sum to 1.0 each state/year combination
    prod_sums = well_avg_gdf.groupby(["state_code", "year"])["prod_annual_rel_emi"].sum()  # get sums to check normalization
    ...
    count_sums = well_avg_gdf.groupby(["state_code", "year"])["count_annual_rel_emi"].sum()  # get sums to check normalization
    ...
    # Check that relative emissions sum to 1.0 each state/year_month combination
    prod_sums = well_avg_gdf.groupby(["state_code", "year_month"])["prod_rel_emi"].sum()  # get sums to check normalization
    assert np.isclose(prod_sums, 1.0, atol=1e-8).all(), f"Relative emissions do not sum to 1 for each year and state; {prod_sums}"  # assert that the sums are close to 1

    count_sums = well_avg_gdf.groupby(["state_code", "year_month"])["count_rel_emi"].sum()  # get sums to check normalization
    assert np.isclose(count_sums, 1.0, atol=1e-8).all(), f"Relative emissions do not sum to 1 for each year and state; {count_sums}"  # assert that the sums are close to 1

    # Create the average relative emission with 50% weights
    well_avg_gdf = (well_avg_gdf
                    .assign(annual_rel_emi=lambda df: 0.5 * (df['prod_annual_rel_emi'] + df['count_annual_rel_emi']))
                    .assign(rel_emi=lambda df: 0.5 * (df['prod_rel_emi'] + df['count_rel_emi']))
                    .drop(columns={'prod_annual_rel_emi', 'count_annual_rel_emi', 'prod_rel_emi', 'count_rel_emi'})
                    )
    
    # Check that relative emissions sum to 1.0 each state/year combination
    avg_sums = well_avg_gdf.groupby(["state_code", "year"])["annual_rel_emi"].sum()  # get sums to check normalization
    ...
    # Check that relative emissions sum to 1.0 each state/year_month combination
    avg_sums = well_avg_gdf.groupby(["state_code", "year_month"])["rel_emi"].sum()  # get sums to check normalization
    assert np.isclose(avg_sums, 1.0, atol=1e-8).all(), f"Relative emissions do not sum to 1 for each year and state; {avg_sums}"  # assert that the sums are close to 1

    # Output Proxy Parquet Files
    well_avg_gdf = well_avg_gdf.astype({'year': int})
