"""
Name:                   pet_hf_comp_emis.py
Date Last Modified:     2024-07-24
Authors Name:           A. Burnette (RTI International)
Purpose:                Mapping of petroleum (HF Workovers) - mobile emissions to State,
                        Year, emissions format
Input Files:            Completions+Workovers_StateEstimates_2024.xlsx
                        [Petro_State_HFWOs_CH4 tab]
...
def get_pet_hf_work_inv_data(input_path, output_path):
    """
    This function will read in the CH4 emissions from Petroleum HF Workovers and
    generate state emissions table with state, yeear, and CH4 emissions in kt.
    """
    emi_df = pd.read_excel(
        inventory_workbook_path,
        sheet_name="Petro_State_HFWOs_CH4",
        nrows=57,
        usecols="A:AI",
        index_col=None
        )
...
# INPUT PATHS
inventory_workbook_path = ghgi_data_dir_path / "Petroleum and Natural Gas/Completions+Workovers_StateEstimates_2024.xlsx"

# OUTPUT PATHS
output_path = tmp_data_dir_path / "pet_hf_work_emi.csv"

# %% STEP 3. Function Calls

get_pet_hf_work_inv_data(
    inventory_workbook_path,
    output_path
    )
