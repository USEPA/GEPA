...
Date Last Modified:     2024-07-16
Authors Name:           A. Burnette (RTI International)
Purpose:                Mapping of petroleum (HF Completions) - mobile emissions to State, Year,
                        emissions format
...
def get_pet_hf_comp_inv_data(input_path, output_path):
    """
    This function will read in the CH4 emissions from Petroleum HF Completions and
    generate state emissions table with state, yeear, and CH4 emissions in kt.
    """
    emi_df = pd.read_excel(
        inventory_workbook_path,
        sheet_name="Petro_State_HFComp_CH4",
        nrows=57,
        usecols="A:AI",
        index_col=None
        )
...
# OUTPUT PATHS
output_path = tmp_data_dir_path / "pet_hf_comp_emi.csv"

# %% STEP 3. Function Calls

get_pet_hf_comp_inv_data(
    inventory_workbook_path,
    output_path
    )
